...
// Shared customer list (mirrors customers page)
const customerList = [
  { id: 'C001', name: 'Café Central', phone: '+351 21 555 0001', address: 'Rua Augusta 45, Lisboa', driver: 'Miguel' },
  { id: 'C002', name: 'Restaurante O Forno', phone: '+351 21 555 0002', address: 'Av. da Liberdade 120, Lisboa', driver: 'Ana' },
  { id: 'C003', name: 'Padaria Estrela', phone: '+351 21 555 0003', address: 'Rua do Ouro 78, Lisboa', driver: 'Miguel' },
  { id: 'C004', name: 'Hotel Lisboa', phone: '+351 21 555 0004', address: 'Praça do Comércio 1, Lisboa', driver: 'Carlos' },
  { id: 'C005', name: 'Supermercado Sol', phone: '+351 21 555 0005', address: 'Estrada de Benfica 500, Lisboa', driver: 'Ana' },
  { id: 'C006', name: 'Cantina Universitária', phone: '+351 21 555 0006', address: 'Campus Universitário, Lisboa', driver: 'Carlos' },
];

// Shared product list (mirrors products page)
const productList = [
  { id: 'P001', name: 'Farinha T65', unit: 'kg', sellingPrice: 1.20 },
  { id: 'P002', name: 'Manteiga', unit: 'kg', sellingPrice: 6.00 },
  { id: 'P003', name: 'Pão de Forma', unit: 'unit', sellingPrice: 1.20 },
  { id: 'P004', name: 'Croissant', unit: 'unit', sellingPrice: 1.50 },
  { id: 'P005', name: 'Leite', unit: 'L', sellingPrice: 1.40 },
  { id: 'P006', name: 'Baguette', unit: 'unit', sellingPrice: 0.80 },
  { id: 'P007', name: 'Fermento', unit: 'kg', sellingPrice: 3.20 },
  { id: 'P008', name: 'Bolo de Mel', unit: 'unit', sellingPrice: 3.50 },
];
...
const mockOrders: Order[] = [
  {
    id: 'ORD-001', customerId: 'C001', customer: 'Café Central', customerPhone: '+351 21 555 0001',
    customerAddress: 'Rua Augusta 45, Lisboa',
    items: [
      { productId: 'P003', name: 'Pão de Forma', qty: 20, unit: 'un', price: 1.20 },
      { productId: 'P004', name: 'Croissant', qty: 30, unit: 'un', price: 0.90 },
    ],
    total: 51.00, status: 'pending', date: '2026-07-22', deliveryDate: '2026-07-23',
    driver: 'Miguel', notes: 'Deliver before 8am', paymentStatus: 'pending',
  },
  {
    id: 'ORD-002', customerId: 'C002', customer: 'Restaurante O Forno', customerPhone: '+351 21 555 0002',
    customerAddress: 'Av. da Liberdade 120, Lisboa',
    items: [
      { productId: 'P006', name: 'Baguette', qty: 40, unit: 'un', price: 0.80 },
      { productId: 'P003', name: 'Pão de Forma', qty: 25, unit: 'un', price: 1.10 },
    ],
    total: 59.50, status: 'confirmed', date: '2026-07-22', deliveryDate: '2026-07-23',
    driver: 'Ana', notes: '', paymentStatus: 'paid',
  },
  {
    id: 'ORD-003', customerId: 'C003', customer: 'Padaria Estrela', customerPhone: '+351 21 555 0003',
    customerAddress: 'Rua do Ouro 78, Lisboa',
    items: [
      { productId: 'P001', name: 'Farinha T65', qty: 50, unit: 'kg', price: 0.85 },
      { productId: 'P007', name: 'Fermento', qty: 10, unit: 'kg', price: 3.20 },
      { productId: 'P002', name: 'Manteiga', qty: 20, unit: 'kg', price: 6.50 },
    ],
  ...
  {
    id: 'ORD-004', customerId: 'C004', customer: 'Hotel Lisboa', customerPhone: '+351 21 555 0004',
    customerAddress: 'Praça do Comércio 1, Lisboa',
    items: [
      { productId: 'P003', name: 'Pão de Forma', qty: 15, unit: 'un', price: 2.10 },
      { productId: 'P008', name: 'Bolo de Mel', qty: 20, unit: 'un', price: 1.80 },
    ],
    total: 67.50, status: 'delivered', date: '2026-07-21', deliveryDate: '2026-07-21',
    driver: 'Carlos', notes: '', paymentStatus: 'paid',
  },
  {
    id: 'ORD-005', customerId: 'C005', customer: 'Supermercado Sol', customerPhone: '+351 21 555 0005',
    customerAddress: 'Estrada de Benfica 500, Lisboa',
    items: [
      { productId: 'P003', name: 'Pão de Forma', qty: 100, unit: 'un', price: 1.20 },
      { productId: 'P008', name: 'Bolo de Mel', qty: 30, unit: 'un', price: 3.50 },
    ],
    total: 225.00, status: 'ready', date: '2026-07-21', deliveryDate: '2026-07-22',
    driver: 'Ana', notes: 'Call on arrival', paymentStatus: 'pending',
  },
  {
    id: 'ORD-006', customerId: 'C001', customer: 'Café Central', customerPhone: '+351 21 555 0001',
    customerAddress: 'Rua Augusta 45, Lisboa',
    items: [{ productId: 'P004', name: 'Croissant', qty: 10, unit: 'un', price: 0.90 }],
...
const emptyNewOrder = {
  customerId: '', customerPhone: '', customerAddress: '',
  deliveryDate: '', driver: '', notes: '', paymentStatus: 'pending' as const,
  items: [{ productId: '', name: '', qty: 1, unit: 'un', price: 0 }],
};
...
export default function OrdersPage() {
  ...
  // When customer is selected in create modal, auto-fill phone/address/driver
  const handleCustomerSelect = (customerId: string) => {
    ...
    if (c) {
      ...
      setNewOrder(p => ({ ...p, customerId: '', customerPhone: '', customerAddress: '', driver: '' }));
    }
  };

  // When product is selected in item row, auto-fill name/unit/price
  const handleProductSelect = (idx: number, productId: string) => {
    const prod = productList.find(p => p.id === productId);
    if (prod) {
      setNewOrder(p => ({
        ...p,
        items: p.items.map((it, i) => i === idx ? { ...it, productId, name: prod.name, unit: prod.unit, price: prod.sellingPrice } : it),
      }));
    }
  };

  const handleEditProductSelect = (idx: number, productId: string) => {
    const prod = productList.find(p => p.id === productId);
    if (prod) {
      setEditOrder(p => p ? {
        ...p,
  ...
  const handleCreateOrder = () => {
    const customer = customerList.find(c => c.id === newOrder.customerId);
    if (!customer || !newOrder.deliveryDate) return;
                    ...
                    <tr key={order.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="table-cell">
                        <div className="flex items-center gap-1">
                          <button onClick={() => setSelected(order)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="View">
                            <Eye size={15} />
                          </button>
                          <button onClick={() => openEdit(order)} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Edit">
                            <Edit2 size={15} />
                          </button>
                          <button onClick={() => setDeleteConfirm(order.id)} className="p-1.5 rounded-lg hover:bg-danger/10 text-muted-foreground hover:text-danger transition-colors" title="Delete">
                            <Trash2 size={15} />
                          </button>
                        </div>
                      </td>
            ...
            <div className="bg-card rounded-2xl shadow-xl w-full max-w-lg p-6 slide-up max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      onClick={() => setNewOrder(p => ({ ...p, items: [...p.items, { productId: '', name: '', qty: 1, unit: 'un', price: 0 }] }))}
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={!newOrder.customerId || !newOrder.deliveryDate}
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 fade-in" onClick={() => setShowEditModal(false)}>
            <div className="bg-card rounded-2xl shadow-xl w-full max-w-lg p-6 slide-up max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const c = customerList.find(c => c.id === e.target.value);
                        if (c) setEditOrder(p => p ? { ...p, customerId: c.id, customer: c.name, customerPhone: c.phone, customerAddress: c.address } : null);
                      }}
                      className="input-field"
                    >
                      {customerList.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                ...
                {/* Edit Items with product dropdown */}
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-medium text-foreground">Order Items</label>
                    <button
                      onClick={() => setEditOrder(p => p ? { ...p, items: [...p.items, { productId: '', name: '', qty: 1, unit: 'un', price: 0 }] } : null)}
                      className="text-xs text-primary hover:underline flex items-center gap-1"
                    >
                      <Plus size={12} /> Add Item
                    </button>
                      ...
                      <div key={idx} className="space-y-1.5 p-3 bg-muted/20 rounded-lg">
                        <div className="flex gap-2">
                          <select
                            className="input-field flex-1 text-xs"
                            value={item.productId}
                            onChange={(e) => handleEditProductSelect(idx, e.target.value)}
                          >
                            <option value="">Select product...</option>
                            {productList.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                          </select>
